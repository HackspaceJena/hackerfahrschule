...
\begin{frame}
	...
	\begin{columns}[c]
		\column[c]{4cm}
			\includegraphics[scale=0.6]{media/tux.png}
		\column{6cm}
			\begin{block}{}
				\begin{itemize}
					\item Linux (im All.) ist eine Sammlung von Programmen
					\item Linux (im Spez.) ist der Betriebssystemkern
					\item Wurde ursprünglich 1991 von Linus Torvalds veröffentlicht
					\item GPL2
					\item Gemeinsam mit den um Richard Stallman entwickelten GNU-Programmen $\rightarrow$ GNU/Linux
				\end{itemize}
			\end{block}
	\end{columns}
\end{frame}
			...
			\item Distribution:
				\begin{itemize}
					...
					\item Beispiele: Ubuntu, Debian, openSuSE/SuSE, RedHat, 
						Fedora, Slackware, Gentoo \dots{}
				\end{itemize}
			\item Aber: Komplett selbst zusammenstellen ist auch möglich 
				(z.B. \textbf{L}inux \textbf{F}rom \textbf{S}cratch $\rightarrow$ LFS)
		\end{itemize}
	\end{block}
	\begin{block}{Bezugsquellen}
		\begin{itemize}
			\item Download aus dem Internet
			\item Kauf eines Paketes oder einer Zeitschrift
